...
export class CommandBridge {
     ...
     */
    async openFile(filePath: string, viewColumn?: number): Promise<any> {
     ...
     * Close a file
     */
    async closeFile(filePath: string): Promise<any> {
        const uri = vscode.Uri.file(filePath);
     ...
     * Save a file
     */
    async saveFile(filePath?: string): Promise<any> {
        if (filePath) {
            const uri = vscode.Uri.file(filePath);
            const document = await vscode.workspace.openTextDocument(uri);
     ...
     */
    async getText(filePath: string, startLine?: number, endLine?: number): Promise<any> {
        const uri = vscode.Uri.file(filePath);
        const document = await vscode.workspace.openTextDocument(uri);
     ...
     */
    async runTerminalCommand(command: string, terminalName?: string): Promise<any> {
     ...
     * Get workspace files
     */
    async getWorkspaceFiles(pattern?: string): Promise<any> {
     ...
     * Format document
     */
    async formatDocument(filePath?: string): Promise<any> {
        let document: vscode.TextDocument;
