...
async function activate(context) {
    ...
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('echo.restartMCPServer', async () => {
        await restartMCPServer();
    }));
    context.subscriptions.push(vscode.commands.registerCommand('echo.stopMCPServer', () => {
        stopMCPServer();
    }));
    context.subscriptions.push(vscode.commands.registerCommand('echo.showMCPLogs', () => {
        outputChannel.show();
    }));
    context.subscriptions.push(vscode.commands.registerCommand('echo.checkMCPHealth', async () => {
...
// ============================================================================
async function handleChatRequest(request, stream, token) {
...
async function registerLanguageModelTools() {
    ...
    for (const tool of availableTools) {
        try {
            ...
            const disposable = vscode.lm.registerTool(`echo_${tool.name}`, {
...
async function startMCPServer() {
    ...
    // Spawn Node process
    try {
        mcpServerProcess = (0, child_process_1.spawn)(nodePath, [serverPath], {
